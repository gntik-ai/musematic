# Auth Bounded Context — Authentication, Authorization, and Session Management

!!! info "Source spec"
    [014-auth-bounded-context](https://github.com/gntik-ai/musematic/tree/main/specs/014-auth-bounded-context) · Status from spec: **Draft** · Created: 2026-04-11

## Purpose

Implement authentication (email/password with Argon2id), JWT token management (RS256), session management (Redis-backed), MFA (TOTP), lockout/throttling, RBAC engine with 10 roles, purpose-bound authorization, and service account API key management.

## How it works

TODO(andrea): summarise the implementation approach (2–4 sentences)
from [plan.md](https://github.com/gntik-ai/musematic/blob/main/specs/014-auth-bounded-context/plan.md) or the
`apps/control-plane/src/platform/auth/` bounded context.
...
- [spec.md](https://github.com/gntik-ai/musematic/blob/main/specs/014-auth-bounded-context/spec.md) — functional
  requirements and acceptance scenarios
- [plan.md](https://github.com/gntik-ai/musematic/blob/main/specs/014-auth-bounded-context/plan.md) — implementation
  approach
- [tasks.md](https://github.com/gntik-ai/musematic/blob/main/specs/014-auth-bounded-context/tasks.md) — task breakdown
  (if present)
