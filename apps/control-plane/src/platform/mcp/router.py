...
def _require_operator(principal: dict[str, Any]) -> tuple[UUID, UUID]:
    roles = principal.get("roles")
    if not isinstance(roles, list):
        raise AuthorizationError("UNAUTHORIZED", "Operator role required")
    allowed = {"owner", "admin", "platform_operator", "operator"}
...
async def register_server(
    ...
    workspace_id, subject = _require_operator(principal)
...
async def list_servers(
    ...
    workspace_id, _subject = _require_operator(principal)
