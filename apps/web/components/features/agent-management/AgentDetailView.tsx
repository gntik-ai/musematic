    ...
    <div className="space-y-6">
      ...
      <Tabs>
        ...
        {activeTab === "overview" ? (
          <TabsContent className="grid gap-4 lg:grid-cols-[1.15fr,0.85fr]">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Core metadata and purpose at a glance.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4 text-sm">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground">
                    {agent.approach ?? "No approach documented."}
                  </p>
                </div>
                <div>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Current visibility and reasoning profile.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4 text-sm">
                <p>
                ...
                <p>
                  <span className="font-medium text-foreground">Reasoning modes:</span>{" "}
                  {agent.reasoning_modes.join(", ")}
                </p>
                <p>
                  <span className="font-medium text-foreground">Visibility patterns:</span>{" "}
                  {agent.visibility_patterns.length}
                </p>
              </CardContent>
            </Card>
          </TabsContent>
          ...
          <TabsContent>
            <div className="flex justify-start">
              <button
                ...
                onClick={() =>
                  router.push(
                    `/agent-management/${encodeURIComponent(agent.fqn)}/revisions`,
                  )
                }
              >
          ...
          <TabsContent>
            <EmptyState
              description={`The ${activeTab} workspace is wired and ready for its dedicated implementation phase.`}
              title={`${activeTab.charAt(0).toUpperCase()}${activeTab.slice(1)} coming next`}
            />
          </TabsContent>
        )}
      </Tabs>
