...
export function DLPRulesForm({ workspaceId, settings }: { workspaceId: string; settings: WorkspaceSettings }) {
  ...
  return (
    <Card>
      <CardHeader><CardTitle>DLP rules</CardTitle></CardHeader>
      <CardContent>
        <form
          className="space-y-4"
          onSubmit={(event) => {
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <Label htmlFor="dlp-enabled">Workspace DLP overrides enabled</Label>
          </div>
          <div className="space-y-2">
            <Label htmlFor="dlp-rule-ids">Rule IDs</Label>
            <Textarea id="dlp-rule-ids" onChange={(event) => setRulesText(event.target.value)} value={rulesText} />
          </div>
          <Button disabled={mutation.isPending} type="submit">Save DLP rules</Button>
        </form>
      </CardContent>
    </Card>
  );
