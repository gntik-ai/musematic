...
- The root pnpm workspace includes `apps/web-status`; no second install is required.
...
cd tests/e2e
uv run pytest journeys/test_j21_platform_state.py -v

# Evaluator scenario authoring + digital twin
uv run pytest journeys/test_j07_evaluator.py -v

# Discovery hypotheses + experiments + evidence
uv run pytest journeys/test_j09_scientific_discovery.py -v

# Component suites
uv run --extra e2e pytest suites/platform_state/ -v
uv run --extra e2e pytest suites/simulation_ui/ -v
uv run --extra e2e pytest suites/discovery_ui/ -v
```
...
# Static status-site coverage lives in the E2E platform-state suite.
cd tests/e2e && uv run --extra e2e pytest suites/platform_state/ -v
```
...
```bash
cd apps/web && pnpm test:i18n-parity
```
...
- **Email hard bounces**: `platform.notifications` currently has no inbound email-provider bounce hook. Follow-up TODO: wire the future bounce callback to mark matching rows in `status_subscriptions` as `health='unhealthy'` when the provider classifies a hard bounce. Until that hook exists, operators can remediate hard bounces manually:

  ```sql
  UPDATE status_subscriptions
  SET health = 'unhealthy', updated_at = now()
  WHERE channel = 'email'
    AND lower(target) = lower('bounced@example.com')
    AND health = 'healthy';
  ```
