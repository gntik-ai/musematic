...
- [X] T041 [P] Write `services/simulation-controller/.golangci.yml` — enable: `errcheck`, `govet`, `staticcheck`, `unused`, `gosec`, `gocyclo (max 15)`, `dupl`; run `make lint` and fix all warnings
...
- [X] T043 Run `go test ./... -coverprofile=coverage.out` in `services/simulation-controller/`, open coverage report — verify ≥ 95% coverage; add missing unit tests for any uncovered branches in `sim_manager`, `artifact_collector`, `ate_runner`, `event_streamer` until threshold is met
