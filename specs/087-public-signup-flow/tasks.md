...
- [x] T012 [W12B] Modify `apps/web/lib/api/auth.ts` per plan.md correction §7: add 4 new methods after the existing OAuth methods at lines 169-200:
...
- [x] T044 [W12C] Author `tests/e2e/journeys/test_j19_new_user_signup.py` per FR-599 + plan.md design Track C: the J19 journey covers all 5 user stories end-to-end:
...
- [x] T045 [W12C] Modify `tests/e2e/journeys/test_j02_creator_to_publication.py` per FR-599: extend the existing journey (already extended in features 085 + 086) with an OPTIONAL OAuth-signup pre-step. When the test parameter `signup_method=oauth_github`, the creator's account is provisioned via GitHub OAuth instead of email+password; the rest of the journey proceeds unchanged. +3 assertion points.
- [x] T046 [W12C] Modify `tests/e2e/journeys/test_j03_consumer_discovery_execution.py` per FR-599: same pattern as T045 — optional OAuth-signup pre-step with `signup_method=oauth_google`. +3 assertion points.
