...
- [ ] T001 Create new bounded-context directory `apps/control-plane/src/platform/localization/` with subdirs `services/` and `tooling/`; add empty `__init__.py` to each; follow the standard BC layout from constitution § Bounded Context Structure
...
- [ ] T006 [P] Add SQLAlchemy models to `apps/control-plane/src/platform/localization/models.py`: `UserPreferences`, `LocaleFile` (FK columns to `users.id` and `workspaces.id` as typed FKs since these ARE specific tables; this is NOT polymorphic like feature 082's tagging substrate)
...
- [ ] T068 [P] Create `apps/web/lib/api/preferences.ts`: typed wrappers over `/api/v1/me/preferences`; TanStack Query hook factories `useUserPreferences`, `useUpdatePreferences` (with optimistic update + rollback)
- [ ] T069 [P] Create `apps/web/lib/api/locales.ts`: typed wrappers over `/api/v1/locales/*` and `/api/v1/admin/locales`; hook factories `useLocaleFile`, `useAvailableLocales`, `usePublishLocaleFile`
