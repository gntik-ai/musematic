...
- [X] T052 [P] Validate test coverage ≥ 95%: run `pnpm test --coverage --reporter=lcov` scoped to `components/features/conversations/`, `components/features/goals/`, `lib/hooks/use-conversation*.ts`, `lib/hooks/use-message*.ts`, `lib/hooks/use-branch*.ts`, `lib/hooks/use-goal*.ts`, `lib/stores/conversation-store.ts`; add missing tests for any component below threshold
