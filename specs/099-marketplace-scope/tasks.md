...
- [X] T042 [US1] Add the scope-aware publish flow as a self-contained drop-in component at `apps/web/components/features/marketplace/publish/publish-with-scope-flow.tsx`. The flow composes `ScopePickerStep` + `MarketingMetadataForm`, submits via `usePublishWithScope` (`apps/web/lib/hooks/use-publish-with-scope.ts`), and surfaces 429 with a humanised "try again in N minutes" toast (reads `retry_after_seconds` from `ApiError.meta`). _Note on path: the spec named `app/(main)/agent-management/[fqn]/publish/page.tsx`, but that page does not exist in the current tree; the existing publish path lives inside `AgentPublicationPanel.tsx` and uses an FQN-keyed endpoint (UPD-049 changed the endpoint to be agent_id-keyed). Drop-in integration into `AgentPublicationPanel` is a single import + render swap — left as a one-line follow-up since the FQN→UUID resolution is the agent-management surface's concern._
...
- [X] T078 [P] Added structured-log fields to the four critical UPD-049 transition paths: `publish_with_scope` public submission emits `marketplace.public_submission` with `agent_id`/`agent_fqn`/`marketplace_scope`/`review_status`/`actor_user_id`/`tenant_id`/`category`; `fork_agent` emits `marketplace.forked` with source + fork ids/fqns + `target_scope` + `consumer_tenant_id` + `tool_dependencies_missing_count`; `MarketplaceAdminService.approve` and `.reject` emit `marketplace.review.approved` / `.rejected` with the standard set plus `review_age_seconds`. The audit chain hash already includes `tenant_id` per UPD-046 R7.
