...
| SC-016 | Partially verified | Metrics are implemented in Python/Go; dashboard JSON is valid with 8 panels and references the required Vault metrics. Live Grafana load and scrape per pod are pending. |
...
| SC-019 | Verified locally | `scripts/generate-env-docs.py` classifies `PLATFORM_VAULT_TOKEN` and SecretID-related refs as sensitive and emits the FR-700 "Never log; never persist outside Vault." annotation. |
...
python -m pytest scripts/tests/test_generate_env_docs.py scripts/tests/test_check_secret_access.py scripts/tests/test_check_admin_role_gates.py -q
