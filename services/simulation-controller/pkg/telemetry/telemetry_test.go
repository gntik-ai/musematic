...
func TestSetupWithoutEndpointReturnsNoopShutdown(t *testing.T) {
	...
	shutdown, err := Setup(context.Background(), "simulation-controller", "")
	if err != nil {
...
func TestSetupWithEndpointConfiguresTelemetry(t *testing.T) {
	...
	shutdown, err := Setup(context.Background(), "simulation-controller", "http://otel-collector:4317")
	if err != nil {
		t.Fatalf("Setup() error = %v", err)
	}
...
func TestSetupPropagatesExporterErrors(t *testing.T) {
	...
	if _, err := Setup(context.Background(), "simulation-controller", "otel-collector:4317"); !errors.Is(err, traceErr) {
	...
	if _, err := Setup(context.Background(), "simulation-controller", "otel-collector:4317"); !errors.Is(err, metricErr) {
...
func TestSetupPropagatesResourceMergeError(t *testing.T) {
	...
	if _, err := Setup(context.Background(), "simulation-controller", "otel-collector:4317"); !errors.Is(err, expectedErr) {
