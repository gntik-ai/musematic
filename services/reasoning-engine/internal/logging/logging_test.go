...
func TestContextHandlerAddsStructuredFields(t *testing.T) {
	...
	logger := slog.New(NewContextHandler(NewJSONHandler(&buf), "reasoning-engine", "platform-execution"))
	...
	if payload["level"] != "info" || payload["message"] != "reasoning completed" {
...
func TestWithGRPCMetadataExtractsCanonicalFields(t *testing.T) {
	...
	logger.InfoContext(WithGRPCMetadata(ctx), "reasoning grpc")
...
func TestContextHandlerAttrsGroupsAndConfigure(t *testing.T) {
	if logger := Configure("reasoning-engine", "platform-execution"); !logger.Enabled(context.Background(), slog.LevelInfo) {
		t.Fatalf("configured logger should be enabled for info")
	}

	var buf bytes.Buffer
	handler := NewContextHandler(NewJSONHandler(&buf), "reasoning-engine", "platform-execution").
		WithAttrs([]slog.Attr{slog.String("component", "grpc")}).
		WithGroup("request")

	slog.New(handler).InfoContext(context.Background(), "grouped", "method", "/reasoning.Reasoning/Select")
