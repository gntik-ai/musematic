...
- 021-agent-registry-ingest: Added agent registry bounded context — package upload/validation pipeline (path traversal, symlinks, size, manifest schema), SHA-256 immutable revisions in MinIO, FQN system (namespace:local_name globally unique), zero-trust default visibility (visibility_agents=[]), FQN pattern discovery with workspace grant union, lifecycle state machine (draft→validated→published↔disabled, published→deprecated→archived) with full audit, maturity classification (Levels 0–3), async Qdrant embedding (eventual consistency ~30s), synchronous OpenSearch indexing with needs_reindex retry, 3 Kafka events on registry.events
- Python 3.12+ + FastAPI 0.115+, Pydantic v2, SQLAlchemy 2.x async + opensearch-py 2.x async (marketplace-agents index) + qdrant-client 1.12+ async gRPC (agent_embeddings) + aioboto3 (agent-packages bucket) + httpx 0.27+ (embedding API) + aiokafka 0.11+ (registry.events) (021-agent-registry-ingest)
