...
- 024-interactions-conversations: Added interactions and conversations bounded context — conversation CRUD (workspace-scoped, soft-delete), bounded interactions with 8-state machine (initializing/ready/running/waiting/paused/completed/failed/canceled), causal message ordering via parent_message_id DAG, mid-process message injection, workspace goal messages (super-context for context engineering, `workspace.goal` topic), attention requests (out-of-band agent urgency on `interaction.attention` topic, §XIII), conversation branching (copy-on-branch) and merging (append with conflict flag), 3 internal interfaces (`get_goal_messages`, `get_conversation_history`, `check_subscription_access`)
- Python 3.12+ + FastAPI 0.115+, Pydantic v2, SQLAlchemy 2.x async + aiokafka 0.11+ (3 topics: `interaction.events`, `workspace.goal`, `interaction.attention`) (024-interactions-conversations)
