...
## Recent Changes
- 017-login-auth: Added login and auth UI — multi-step login (credentials→MFA→lockout), password reset flow (anti-enumeration + per-rule strength), MFA enrollment dialog (QR code + verify + recovery codes + acknowledgment), 6 TanStack Query mutations, lockout countdown hook, `?redirectTo` deep link support
- 016-accounts-bounded-context: Added accounts bounded context — self-registration (anti-enumeration, SHA-256 tokens), email verification, signup modes, admin approval workflow, invitation-based onboarding, lifecycle state machine, 8 admin actions, workspace provisioning event, 17 REST endpoints, 15 Kafka event types
